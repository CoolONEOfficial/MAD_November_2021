...
//        }.padding(.horizontal, 24).padding(.vertical, 16).background(RoundedRectangle(cornerRadius: 8).strokeBorder(lineWidth: 0.5).foregroundColor(.darkGray))
    ...
    var questionContent: some View {
        Text("\(vm.questionIndex + 1) of \(vm.questions.count)").font(.plain).foregroundColor(.darkGray).padding(.bottom, 8)
        Text(vm.currentQuestion?.model.meanings.first?.definitions.first?.definition ?? "").font(.title).multilineTextAlignment(.center).padding(.bottom, 24)
    ...
    var test: some View {
        RoundedRectangle(cornerRadius: 3).fill(Color.gray).height(6).overlay(alignment: .leading) {
            GeometryReader { read in
                RoundedRectangle(cornerRadius: 3).fill(Color.orange).height(6).width(read.size.width / 5 * CGFloat(vm.seconds))
            }
        }.padding(.bottom, 37)
    }
